...
export function CockpitCards({ data }: CockpitCardsProps) {
  ...
  const cards = [
    ...
    {
      id: "compromissos",
      title: "Gastos / Compromissos do Mês",
      ...
      status: "warning",
      tooltip:
        "Total de despesas e parcelas de empréstimos registradas no período atual.",
    },
    {
      id: "projecao",
        ...
        {cards.map((card, index) => (
          <Tooltip key={card.id}>
            <TooltipTrigger asChild>
              <div
                className={cn(
                  "glass-card p-4 flex flex-col justify-between gap-2 transition-all hover:scale-[1.02] cursor-help",
                  statusColors[card.status as keyof typeof statusColors],
                  // Deixar o primeiro card (Saldo / Patrimônio) mais largo, como no modelo
              ...
              >
                <div className="flex items-start justify-between gap-2">
                  <div className="flex flex-col gap-1 min-w-0">
                    ...
                    <span className={cn("text-lg md:text-xl font-semibold truncate", card.color)}>
                      {card.trend === "down" && "-"}
                      {card.trend === "up" && "+"}
                      {card.value}
                    </span>
                  ...
                  <div
                    className={cn(
                      "shrink-0 rounded-2xl p-2.5 md:p-3",
                      card.bgColor,
                    )}
