...
const Index = () => {
  ...
  const totalReceitas = useMemo(() => {
    return filteredTransacoesV2.filter(t => t.flow === "in" && t.operationType !== 'transferencia').reduce((acc, t) => acc + t.amount, 0);
  }, [filteredTransacoesV2]);

  const totalDespesas = useMemo(() => {
    return filteredTransacoesV2.filter(t => t.flow === "out" && t.operationType !== 'transferencia').reduce((acc, t) => acc + t.amount, 0);
  }, [filteredTransacoesV2]);

  const receitasMes = useMemo(() => {
    const now = new Date();
    return filteredTransacoesV2
      .filter(t => t.flow === "in" && t.operationType !== 'transferencia' && new Date(t.date).getMonth() === now.getMonth() && new Date(t.date).getFullYear() === now.getFullYear())
  ...
  const despesasMes = useMemo(() => {
    const now = new Date();
    return filteredTransacoesV2
      .filter(t => t.flow === "out" && t.operationType !== 'transferencia' && new Date(t.date).getMonth() === now.getMonth() && new Date(t.date).getFullYear() === now.getFullYear())
      .reduce((acc, t) => acc + t.amount, 0);
  }, [filteredTransacoesV2]);
  ...
  const heatmapData = useMemo(() => {
    ...
    return Array.from({ length: 31 }, (_, i) => {
      const day = i + 1;
      const dayTransacoes = filteredTransacoesV2.filter(t => {
        const d = new Date(t.date);
        return d.getDate() === day && String(d.getMonth() + 1).padStart(2, '0') === currentMonth && d.getFullYear() === currentYear;
      });
      
      return {
        day,
        receitas: dayTransacoes.filter(t => t.flow === "in" && t.operationType !== 'transferencia').reduce((acc, t) => acc + t.amount, 0),
        despesas: dayTransacoes.filter(t => t.flow === "out" && t.operationType !== 'transferencia').reduce((acc, t) => acc + t.amount, 0),
  ...
  const renderSection = (sectionId: string) => {
    ...
    switch (sectionId) {
      ...
      case "heatmap":
        return <FluxoCaixaHeatmap month={currentMonth} year={currentYear} transacoes={filteredTransacoesV2} />;
      case "indicadores":
